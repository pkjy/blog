    ...
    <div v-show="false">
      ...
      <table>
        <tbody>
          ...
          <tr>
            <td>视频播放APP</td>
            <td>空闲时候开发的项目，主要用于熟悉vue3的语法+cordova打包APP。用cordova打包成APP。后端目前用eggjs写的，支持注册登录等功能。</td>
          </tr>
        </tbody>
      </table>
...
export default defineComponent({
  ...
  methods: {
    throttle(method: any, context?: any) {
      // 用函数节流优化rezise——《高程》
    ...
    renderCanvas() {
      const test = document.getElementById('myCanvas')
      if (test) {
      ...
      this.interval = window.setInterval(loop, 36)
      function Circle(this: any, x: any, y: any, r: any, color: any) {
        this.x = x
        this.y = y
        this.r = r
        this.color = color
        ...
        this.pos = function () {
          const value = this.grow * this.tremble
  ...
  beforeUnmount() {
    ...
    if (domCanvas) document.body.removeChild(domCanvas)
  },
})
</script>
