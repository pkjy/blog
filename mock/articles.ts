...
export const getArticles = (req: Request, res: Response) => {
  ...
  let mockList = articleList.filter(item => {
    ...
    if (title && typeof (title) !== 'object' && item.title.indexOf(title) < 0) return false
    return true
  })
  ...
  const pageList = mockList.filter((_, index) => index < Number(limit) * Number(page) && index >= Number(limit) * (Number(page) - 1))
